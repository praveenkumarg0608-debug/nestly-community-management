...
function App() {
  ...
  const [purchasedMarketItems, setPurchasedMarketItems] = usePersistentState<string[]>('purchasedMarketItems', []);
  ...
  const commonProps = {
    ...
    marketItem,
    setMarketItem,
    purchasedMarketItems,
    setPurchasedMarketItems,
...
function Layout(props: any) {
  ...
  return <div className="app-shell">
    ...
    {modal && <Modal title={modal} marketItem={marketItem} close={()=>{setModal(null);setMarketItem(null)}} toast={toast} onSave={async (data:any)=>{
  ...
  return;
      ...
      if (modal === 'Buy item') {
        if (!token) throw new Error('Please sign in again to complete the purchase.');
        const itemId = marketItem?.id;
        const purchaseKey = itemId ? `id-${itemId}` : `name-${marketItem?.name}`;
        if (itemId) {
          await apiFetch(`/marketplace/${itemId}/buy`, {method:'PATCH'}, token);
          setListings((x:any[]) => x.filter((item:any) => Number(item.id) !== Number(itemId)));
        }
        setPurchasedMarketItems((x:string[]) => x.includes(purchaseKey) ? x : [...x,purchaseKey]);
        toast(`${marketItem?.name || 'Item'} purchased successfully.`);
        return;
      }
...
function Marketplace({setModal,toast,listings,purchasedMarketItems,setMarketItem}:any){
  const items=[
    {name:'Study table',price:'₹2,500',category:'Furniture'},
    {name:'Mountain bicycle',price:'₹8,000',category:'Sports'},
    {name:'2BHK apartment',price:'₹24,000/mo',category:'Rent'},
    {name:'Dining chairs set',price:'₹4,500',category:'Furniture'}
  ];
  const all=[
    ...listings.map((x:any)=>({id:x.id,name:x.name||x.title,price:x.amount||(x.price?`₹${x.price}`:''),category:x.category,description:x.description||'',seller:x.seller||'Greenview resident'})),
    ...items
  ];
  const openBuy=(item:any)=>{
    const key=item.id ? `id-${item.id}` : `name-${item.name}`;
    if(purchasedMarketItems?.includes(key)){ toast(`${item.name} has already been purchased.`); return; }
    setMarketItem(item);
    setModal('Buy item');
  };
  return <div className="content inner">
    <PageHead eyebrow="COMMUNITY MARKETPLACE" title="Buy • Sell • Rent" sub="A trusted space to exchange useful things with neighbours." action={<button className="primary" onClick={()=>setModal('List an item')}><Plus size={17}/> List an item</button>}/>
    <div className="market-grid">
      {all.map((item:any,i:number)=>{const key=item.id?`id-${item.id}`:`name-${item.name}`; const purchased=purchasedMarketItems?.includes(key); return <article className="market" key={`${item.name}-${item.id||i}`}>
        <div className={`market-image m${i%4}`}><span>{item.category}</span></div>
        <div><h2>{item.name}</h2><strong>{item.price}</strong><small>Listed by {item.seller||'Greenview resident'}</small><button className={purchased?'joined':''} onClick={()=>openBuy(item)} disabled={purchased}>{purchased?'Purchased ✓':'Buy now'} {!purchased&&<ArrowRight size={15}/>}</button></div>
      </article>})}
    </div>
  </div>;
}
...
function Modal({title,marketItem,close,toast,onSave}:any){
  ...
  function renderFields(){
    ...
    if(isBuy){
      return <div className="payment-confirm">
        <b>{marketItem?.name || 'Marketplace item'}</b>
        <strong>{marketItem?.price || 'Price not specified'}</strong>
        <small>{marketItem?.category || 'Community marketplace'} · Demo purchase</small>
      </div>;
    }
